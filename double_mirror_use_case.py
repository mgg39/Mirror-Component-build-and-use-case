...
if __name__ == "__main__":
    ...
    node2A = MiddleNode("node2A", tl, "node3A")
    node3A = ReceiverNode("node3A", tl)
    node1B = EmittingNode("node1B", tl)
    node2B = MiddleNode("node2B", tl, "node3B")
    node3B = ReceiverNode("node3B", tl)
    ...
    qc2A.set_ends(node2A, node3A)
    qc1B.set_ends(node1B, node2B)
    qc2B.set_ends(node2B, node3B)


    # counter
    counterA = Counter()
    counterB = Counter()
    node3A.detector.attach(counterA)
    node3B.detector.attach(counterB)
